...
impl UndoHistory {
    ...
    pub fn record_previous_state(&mut self, root: &lang::CodeNode, cursor_position: Option<lang::ID>) {
        let root = root.clone();
        self.undo_stack.push(UndoHistoryCell { root, cursor_position });
        self.redo_stack.clear();
    }

    pub fn undo(&mut self, current_root: &lang::CodeNode,
                cursor_position: Option<lang::ID>) -> Option<UndoHistoryCell> {
        self.redo_stack.push(UndoHistoryCell { root: current_root.clone(), cursor_position });
        self.undo_stack.pop()
    }

    pub fn redo(&mut self, current_root: &lang::CodeNode,
                cursor_position: Option<lang::ID>) -> Option<UndoHistoryCell> {
        let redone_state = self.redo_stack.pop()?;
        self.undo_stack.push(UndoHistoryCell { root: current_root.clone(), cursor_position });
        Some(redone_state)
    }
}