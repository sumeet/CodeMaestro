...
fn all_code<'a>(env_genie: &'a EnvGenie) -> impl Iterator<Item = (CodeLocation, &'a lang::Block)> + 'a {
...
impl<'a> Validator<'a> {
    ...
    fn validate_and_fix_all_code(&mut self) {
        ...
        let problems = all_code(&env_genie)
            .flat_map(|(location, block)| problem_finder.find_problems(location, block));
    ...
    fn fix_return_type(&self, mut block: lang::Block, mut required_return_type: lang::Type) -> lang::Block {
    ...
    fn update_code(&mut self, location: CodeLocation, block: lang::Block) {
        match location {
            ...
            CodeLocation::JSONHTTPClientURLParams(id) => {
                let mut json_http_client = self.env_genie
                    .get_json_http_client(id).unwrap().clone();
...
impl<'a> ProblemFinder<'a> {
    ...
    fn find_problems(&self, location: CodeLocation, block: &lang::Block) -> impl Iterator<Item = Problem> {
        self.find_return_type_problem(location, block).into_iter()
    }

    fn find_return_type_problem(&self, location: CodeLocation, block: &lang::Block) -> Option<Problem> {
        let required_return_type = self.required_return_type(location)?;
        if !required_return_type.matches(&self.returned_type(block)) {
            Some(Problem::InvalidReturnType {
                location,
                block: block.clone(),
                required_return_type,
            })
    ...
    fn required_return_type(&self, location: CodeLocation) -> Option<lang::Type> {
        match location {
            ...
            CodeLocation::JSONHTTPClientURLParams(_) => {
                Some(LIST_OF_FORM_PARAMS.clone())
            },
            CodeLocation::JSONHTTPClientURL(_) => {
                Some(lang::Type::from_spec(&*lang::STRING_TYPESPEC))
            },
            CodeLocation::ChatTrigger(_) | CodeLocation::Script(_) |
                CodeLocation::Test(_) => None,
        }
    }
