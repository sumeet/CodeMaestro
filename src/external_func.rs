...
pub fn to_named_args(func: &lang::Function, args: HashMap<lang::ID, lang::Value>) -> HashMap<String, lang::Value> {
    let mut short_name_by_id : HashMap<lang::ID, String> = func.takes_args().into_iter()
        .map(|argdef| (argdef.id, argdef.short_name))
        .collect();
    args.into_iter()
        .map(|(arg_id, value)| (short_name_by_id.remove(&arg_id).unwrap(), value))
        .collect()
}
