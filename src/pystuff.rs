...
use pyo3::prelude::*;
use pyo3::types::{PyDict, PyIterator, PyObjectRef};
...
impl PyNamespace {
    ...
    fn new(gilguard: &GILGuard) -> Self {
        Self { locals: newdict(gilguard.python()) }
    }
    ...
    fn into_object(self, py: Python) -> PyObject {
        ...
        match (self.env, self.value) {
            ...
            (env, Struct { struct_id, values }) => {
                ...
                let value_by_name: HashMap<&str, ValueWithEnv> =
                    values.into_iter()
                          .map(|(id, value)| {
                ...
                ms.call1((&strukt.name, into_pyobject(value_by_name, py)))
                  .unwrap()
                  .to_object(py)
            }
...
impl PyFunc {
    ...
    fn pyobject_into_struct(&self,
                            ...
                            env: &env::ExecutionEnvironment)
                            -> PyResult<lang::Value> {
        let values: PyResult<HashMap<lang::ID, lang::Value>> =
            strukt.fields
                  .iter()
                  .map(|strukt_field| {
                      let field_ref = try_get_name(pyobjectref, &strukt_field.name)?;
                      Ok((strukt_field.id, self.ex(&field_ref, &strukt_field.field_type, env)))
                  })
                  .collect();
        Ok(lang::Value::Struct { struct_id: strukt.id,
                                 values: values? })
    }
...
fn mix_args_with_locals<'a>(py: Python, args: &'a PyDict, from: PyObject) -> &'a PyDict {
    ...
    mixed_as_obj.getattr(py, "update")
                .unwrap()
                .call1(py, (from,))
                .unwrap();
    mixed
...
fn into_pyobject<K, V, H>(from: HashMap<K, V, H>, py: Python) -> PyObject
    where K: hash::Hash + cmp::Eq + IntoPyObject,
          V: IntoPyObject,
          H: hash::BuildHasher
{
